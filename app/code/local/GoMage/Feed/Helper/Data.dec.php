...
class GoMage_Feed_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function a($k, $c = 0, $s = '')
    {
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, 'key=' . urlencode($k) . '&sku=feed-pro&domains=' . urlencode(implode(',', $this->getAllStoreDomains())) . '&ver=' . urlencode('4.0.0'));
    ...
    public function notify()
    {
        ...
        try {
            ...
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, 'sku=feed-pro&timestamp=' . $timestamp . '&ver=' . urlencode('4.0.0'));
            curl_setopt($ch, CURLOPT_TIMEOUT, 30);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
